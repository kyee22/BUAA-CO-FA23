...
you_file_path = "code.txt";
mark=0
while mark==0:
    input()
    try:
         file_in = open(you_file_path,'r+')
         file_out = open('out.txt','wt')
         mark = 1
    except:
         print(you_file_path + " does not exist. Please try again.")
...
for hexcode in machine:
    ...
    if itype=="J":
        ...
        if mips[0]=='b':
            ...
            print(imm)
            if imm[0]=='1':
                immv = int(imm[1:],2) - 0x01;
                immv = -(~immv & int("0b" + "1" *(len(imm)-1),2))
            else:
                immv = int(imm, 2)
            print(immv)
            linenum= int( (mipscount+1+immv))
            if not linenum in label.keys():
                labelname="label_"+str(labelcount)
                try:
                    out[linenum] = labelname+": "+out[linenum]
